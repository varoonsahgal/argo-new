...
smoke_test() {
  ...
  # Cross-cluster name resolution proof: the workload node name resolves from a
  # mgmt pod (repo-server already proved lab-gitea by cloning hello-reconcile).
  if kmgmt -n "${ARGOCD_NAMESPACE}" exec deploy/argocd-repo-server -- \
       nslookup "${WORKLOAD_NODE}" >/dev/null 2>&1; then
