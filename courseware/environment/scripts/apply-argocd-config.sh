...
main() {
  ...
  # --hide-notes: the chart's NOTES tell readers to port-forward 8080 and read the
  # initial admin Secret, which contradicts the course setup (port 8443, and the
  # password from ~/course/credentials). Participants see this output on every reset.
  helm_args+=(--hide-notes)
