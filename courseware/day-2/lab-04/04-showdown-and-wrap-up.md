...
- **Preview before you apply.** `argocd appset generate` costs three seconds; an unplanned rollout costs an afternoon. Preview → count → apply.
...
- **The dangerous bug is a successful render of the wrong thing.** `missingkey=error` makes a silent empty into a loud, local error. Safer fails more, sooner, on purpose.
