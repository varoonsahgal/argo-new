...
| 2 | **Lab 3, Exercise 2** | Image tag `podinfo:6.16.0` **does not exist** in the registry (newest is `6.15.0`). The promotion always fails with `ImagePullBackOff` → `Degraded` | Run it anyway and teach it as "roll forward failed → `git revert`" |
| 3 | **Lab 3, Exercise 5 Part B** | Committing only `migration.shouldFail: true` never triggers auto-sync (hooks aren't compared). When it does fail, v3.5.2 **retries 5 times by default**, and a pushed fix does not rescue the retrying operation | Pair the flag with a visible change; recover with a roll-forward commit **plus** `argocd app terminate-op` |
...
| 6 | **Lab 1, Exercise 2** | Environments seeded before commit `21a3fc8` lack the chart's rollout annotation: the sync succeeds but the Pod never restarts, and `curl` shows the **old** message | Check before class; if it happens live, it's a great "ConfigMaps don't restart Pods" lesson |
| 7 | **Lab 1, Exercises 2–3** | Two printed commands return the wrong thing: the `curl` `grep` misses podinfo's `"message": "…"` (space after the colon), and the `sed` range drops the ConfigMap's `data:` block | Corrected commands in the Lab 1 walkthrough |
