...
**One rule for port-forwards in this lab: after every sync that replaces Pods, restart the port-forward before you run `curl`.** Here is why.

```mermaid
flowchart LR
    subgraph BEFORE["When you start the port-forward"]
        C1["curl localhost:9898"] --> PF1["kubectl port-forward"] --> P1["Pod A (old version)"]
    end
    subgraph AFTER["After a sync replaces the Pod"]
        C2["curl localhost:9898"] --> PF2["the SAME port-forward"] --> P2["Pod A, shutting down<br/>(still answers for a few seconds,<br/>then the tunnel dies)"]
        P3["Pod B (new version)<br/>nobody is connected to it"]
    end
```

> **💡 Analogy — a phone call, not a phone number.** `kubectl port-forward svc/storefront` looks up the Service once, picks **one** Pod, and holds an open call to that Pod. When a sync replaces the Pod, your call is still connected to the old one. For a few seconds it keeps answering with the **old** version, then the call drops. Hanging up and dialling again (Ctrl-C, then rerun the command) connects you to the new Pod.
>
> **What goes wrong if you skip the restart:** `curl` right after a sync can print the *old* version number, which looks exactly like "the promotion did not work". It did work; you asked the old Pod.
...
2. Observe the order: the **PreSync** migration Job is a task that must finish successfully before the main resources are applied. **Sync waves** are numbered groups; lower numbers run first. After the Job succeeds, Argo CD applies the `ConfigMap` (wave `-1`), then the `Deployment` and `Service` (wave `0`). The Deployment goes `Progressing` → `Healthy`.
   ...
   > **💡 Remember the boarding groups from [Session 4 · Module 1.5](../session-04/01b-sync-order-phases-waves-kinds-names.md).** The PreSync Job is *pre-boarding*: it happens before any group is called. Then group `-1` (the ConfigMap) boards, and only when it is seated does the gate call group `0` (the Service and Deployment).

   ```mermaid
   flowchart LR
       H["PreSync hook<br/>Job storefront-migration<br/>must SUCCEED"] --> W1["Wave -1<br/>ConfigMap storefront"]
       W1 --> W0["Wave 0<br/>Service + Deployment storefront"]
       W0 --> D["Sync finished<br/>Deployment: Progressing, then Healthy"]
   ```
...
4. **Verify the running app.** Wait until `storefront-dev` shows **`Synced` / `Healthy`**. The Service now exists, so you can open the temporary tunnel introduced in Module 1.
   ...
   **Expected:**
   ...
   Keep Terminal A open for later checks. **Ctrl-C** stops the tunnel. After any later sync that replaces the Pod, press **Ctrl-C** in Terminal A and run the same port-forward command again *before* using `curl` — even if the old tunnel still seems to work (see "One rule for port-forwards" above).
...
```mermaid
flowchart LR
    CH["charts/storefront<br/>ONE shared chart"]
    DV["envs/dev/values.yaml<br/>tag, message DEV, 1 replica"]
    SV["envs/staging/values.yaml<br/>tag, message STAGING, 2 replicas"]
    AD["Application storefront-dev"]
    AS["Application storefront-staging<br/>(you write this)"]
    ND["namespace storefront-dev"]
    NS["namespace storefront-staging"]
    CH --> AD
    CH --> AS
    DV --> AD --> ND
    SV --> AS --> NS
...
**Promotion, in one picture:** the only thing that moves between environments is one line, `image.tag`, copied from the dev card to the staging card after dev proves it works.
...
1. In `storefront-gitops`, set `image.tag: "6.15.0"` in `envs/dev/values.yaml`, commit, push, and sync `storefront-dev`. Wait for **`Synced` / `Healthy`**. The sync replaced dev's Pod, so **restart dev's port-forward**: in Terminal A press **Ctrl-C**, then run it again:
   ```bash
   kubectl --context k3d-workload -n storefront-dev port-forward svc/storefront 9898:9898
   ```
   Then, from your command terminal, confirm the new version:
   ```bash
...
2. **Promote** by making the *same* one-line change to `envs/staging/values.yaml`, commit, push. Before syncing, render locally to see the new tag:
   ...
   **Expected** (the second image belongs to the migration Job, which does not change):
   ...
   ```
3. Sync `storefront-staging` and wait for **`Synced` / `Healthy`**. **Restart staging's port-forward**: in Terminal C press **Ctrl-C**, then run it again:
   ...
   **Expected:** `"version": "6.15.0"`. If you see `6.14.1`, your port-forward is still connected to the old Pod — restart it and try again.
