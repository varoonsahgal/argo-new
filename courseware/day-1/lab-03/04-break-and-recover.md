...
```mermaid
flowchart TD
    G["Git commit"] --> R{"Step 1: repo-server renders<br/>chart + values into YAML"}
    R -- "fails" --> A["PART A: rendering failure<br/>Evidence: APP CONDITIONS, ComparisonError<br/>Sync status: Unknown<br/>Cluster: untouched"]
    R -- "works" --> S{"Step 2: application-controller applies<br/>PreSync hook, then waves"}
    S -- "a step fails" --> B["PART B: ordering failure<br/>Evidence: LAST SYNC, Sync failed<br/>Later steps never ran"]
    S -- "every step works" --> OK["Synced"]
```

> **💡 Analogy — a printer and a delivery van.** The repo-server is the printer that turns your order into printed labels. If the printer jams (Part A), no labels exist and the van never leaves; the problem is at the print shop. The application-controller is the van that delivers the labelled boxes in order. If the first stop is closed (Part B, the failed PreSync Job), the van does not continue to the later stops. Both are "the parcel didn't arrive", but you phone a different person.
...
**Recover:** restore the correct `valueFiles` reference **through Git** (a `git revert` of the breaking commit is cleanest), apply, refresh. Confirm the `ComparisonError` clears. If `storefront-staging` then shows `OutOfSync` on one resource (the one you annotated in E4), that is your E4 `Prune=false` commit, which staging (still manual sync) has not applied yet: sync `storefront-staging` to finish.
...
| `storefront-staging` shows `Unknown` / `Unknown` with *`InvalidSpecError … do not match any of the allowed destinations in project 'storefront'`* (even though `kubectl apply` said `created`) | AppProject still permits only `storefront-dev` | Add the `storefront-staging` destination to `projects/storefront.yaml`, commit, re-apply the project, then click **Refresh** on `storefront-staging` (without it, the app can take 20 seconds or more to re-check) |
| First sync of `storefront-dev` fails with `resource apps:Deployment is not permitted in project storefront` (or `batch:Job`) | Lab 2's `namespaceResourceWhitelist` has that kind with the wrong `group`, or is missing it | Fix the entry (`apps` for Deployment, `batch` for Job, `autoscaling` for HPA, `""` for ConfigMap/Service) in `platform-config/projects/storefront.yaml`, commit, re-apply, sync again. Lab 2 E4 step 5 shows a dry-run check for this |
| `curl` shows the **old** version (or old message) right after a successful sync | Your port-forward is still connected to the old Pod, which answers for a few seconds while it shuts down | Press **Ctrl-C** on the port-forward, run it again, then `curl` |
...
| A new **commit** takes a minute or more to show (up to about two minutes in testing) | Argo CD checks Git every 60 seconds, and then needs a moment to render and compare | **Refresh** makes it check Git now; **Hard Refresh** also re-renders the manifests |
...
Cross-check: (1) both `storefront-dev` and `storefront-staging` are `Synced`/`Healthy`; (2) both REVISION values equal the `git rev-parse HEAD` line; (3) each serves (`curl` returns `storefront DEV` / `storefront STAGING` — restart a port-forward first if its Pod was replaced since you started it).

> **If staging's REVISION is one commit behind:** your last few commits changed only dev's files, and Argo CD has not re-checked staging since. Click **Refresh** on `storefront-staging` (or run `argocd app get storefront-staging --refresh`), then run the `kubectl … get applications` command again. Staging stays `Synced` either way, because those commits did not change anything staging renders.
...
**Check your notes against this map** (only after writing your own):

```mermaid
flowchart TD
    Q["Something looks wrong"] --> A1{"Settings: repository or cluster<br/>not Successful?"}
    A1 -- "yes" --> F1["Repository / access<br/>Owner: repo-server, which fetches Git<br/>Fix: the repository Secret (url, username, password)"]
    A1 -- "no" --> A2{"APP CONDITIONS shows<br/>ComparisonError?"}
    A2 -- "yes" --> F2["Rendering<br/>Owner: repo-server<br/>Fix: the repo holding the broken reference or value"]
    A2 -- "no" --> A3{"LAST SYNC says<br/>Sync failed?"}
    A3 -- "yes" --> F3["Synchronization / ordering<br/>Owner: application-controller<br/>Fix: the desired state in Git, then sync"]
    A3 -- "no" --> A4{"Synced, but health is<br/>Degraded or Progressing?"}
    A4 -- "yes" --> F4["Health<br/>Produced by Kubernetes, reported by the application-controller<br/>Fix: usually Git (image tag, probes, replicas)"]
```
