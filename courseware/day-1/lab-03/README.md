...
- **Waves and hooks order a sync — like boarding a plane.** The PreSync migration Job is pre-boarding and must succeed *before* the main resources; then the ConfigMap boards with wave `-1`, and the Deployment/Service with wave `0`. Argo CD waits for each group to be healthy before calling the next. **If an earlier stage fails, later stages never run.**
