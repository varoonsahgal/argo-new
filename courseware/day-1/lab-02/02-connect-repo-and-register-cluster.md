...
**The same table as a picture.** Keep this map open while you work. Every "which context?" question in this module is answered by finding the object on it.

```mermaid
flowchart LR
    subgraph MGMT["k3d-mgmt — the office where Argo CD works"]
        direction TB
        ACD["Argo CD<br/>(namespace argocd)"]
        RS["E1: repository Secret<br/>repo-storefront-gitops"]
        CS["E2 Move C: cluster Secret<br/>cluster-workload<br/>(holds a COPY of the token)"]
    end
    subgraph WL["k3d-workload — the building Argo CD delivers to"]
        direction TB
        SA["E2 Move A: ServiceAccount<br/>argocd-manager"]
        TK["E2 Move A: token Secret<br/>argocd-manager-token"]
        RB["E2 Move A: Roles + RoleBindings<br/>(which rooms the key card opens)"]
    end
    GIT["Gitea: storefront-gitops<br/>(private repository)"]
    ACD -- "reads" --> RS
    ACD -- "reads" --> CS
    RS -. "password lets Argo CD read" .-> GIT
    TK -. "helper copies token + CA" .-> CS
    CS -. "Argo CD signs in as" .-> SA
    SA --- RB
```

> **💡 Analogy — the office and the building.** Argo CD works in an office (the management cluster). It keeps its keys in a drawer in that office (the `argocd` namespace). The key card itself is *made* by the building it opens (the workload cluster), because only that building's security desk can decide which doors a card unlocks. Argo CD never walks over to the building to fetch the card; the helper makes a copy and drops it in the office drawer.

### Before you begin

Continue in the same terminal on your course VM that you used in Module 1. That terminal has the Module 1 files and both Kubernetes contexts. (Run `source ~/argo-lab-env.sh` first if you opened a new terminal.)
...
5. Uses `apply --server-side` to avoid creating a client-side last-applied annotation. It also adds `--force-conflicts`, so re-running the helper replaces values that another tool changed since (for example a lab reset or a `kubectl patch`), instead of refusing.
...
**The whole trip, in order.** Read it top to bottom. Steps 1–3 happen once, when you run the helper in Move C. Steps 4–5 happen every time Argo CD talks to the workload cluster, long after the helper has finished.

```mermaid
sequenceDiagram
    participant H as Helper (your terminal)
    participant W as Workload API server
    participant M as Management cluster (argocd namespace)
    participant C as Argo CD application controller
    H->>W: 1. Read argocd-manager-token (token + CA certificate)
    H->>M: 2. Write cluster Secret cluster-workload with the copied token and CA
    Note over H: 3. The helper exits. Its job is done.
    C->>M: 4. Read cluster-workload
    C->>W: 5. Call https://k3d-workload-server-0:6443, presenting the token
    W-->>C: RBAC decides: allowed in the five namespaces, refused elsewhere
```
...
| E1 helper says `Kubernetes operation failed` and you chose `k3d-workload` | The workload cluster has no `argocd` namespace, so Kubernetes has nowhere to store the Secret. Argo CD runs on management: set `LAB_REPO_CONTEXT='k3d-mgmt'` and run the helper again. |
| Move A prints many lines like `namespaces "argocd-access" not found` | You applied the RBAC file to `k3d-mgmt`. Those namespaces exist only on the workload cluster, so nothing was created. Set `LAB_IDENTITY_CONTEXT='k3d-workload'` and run Move A again. |
...
Two separate keys, two separate locks. The Git key opens the warehouse where the deployment files are kept. The workload key card opens only certain rooms in the building. Neither key does the other one's job.

```mermaid
flowchart LR
    ACD["Argo CD<br/>on k3d-mgmt"]
    subgraph KEYS["Keys in the argocd namespace"]
        K1["repo-storefront-gitops<br/>username + password"]
        K2["cluster-workload<br/>server + token + CA"]
    end
    GIT["Gitea<br/>storefront-gitops (private)"]
    subgraph WL["k3d-workload"]
        OK["storefront-dev, storefront-staging,<br/>storefront-prod, team-a, platform-system<br/>(Roles grant selected writes)"]
        NO["every other namespace<br/>and all cluster-scoped objects<br/>(no RoleBinding: refused)"]
    end
    ACD --> K1 --> GIT
    ACD --> K2 --> OK
    K2 -. "403 Forbidden" .-> NO
...
def main():
    ...
    # Optional: fill a different placeholder file. Module 4 (E5) uses this.
    ...
    home = Path.home()
    if args.file:
        template = Path(args.file).expanduser()
    else:
    ...
    # --force-conflicts: take ownership of fields another tool wrote since
    # (a lab reset, or Stretch C's kubectl patch), instead of refusing to apply.
    kubectl([
        '--context', args.context, 'apply', '--server-side', '--force-conflicts',
...
Module 4 reuses the same helper for its broken records. The optional `--file` argument tells it which placeholder file to fill instead of the `platform-config` template; everything else works the same way.

**Already created the helper from an earlier version of this guide?** Run the setup block again. It replaces the file with this version, which adds `--file` and `--force-conflicts`.
