...
```mermaid
flowchart LR
    REQ["Argo CD wants to create<br/>a Deployment in storefront-dev"] --> G1{"Gate 1: Argo CD's own rules<br/>(AppProject, E4)"}
    G1 -- "not allowed" --> N1["Refused BEFORE the sync starts.<br/>Nothing reaches Kubernetes."]
    G1 -- "allowed" --> G2{"Gate 2: Kubernetes RBAC<br/>on the workload cluster (E3)"}
    G2 -- "not allowed" --> N2["403 Forbidden DURING the sync.<br/>The request reached the cluster."]
    G2 -- "allowed" --> Y["Created"]
```

> **💡 Analogy — two security checks at an airport.** Gate 1 is the airline desk checking your ticket before you ever reach security. Gate 2 is the security scanner itself. If the airline desk turned you away, the scanner never saw you. E3 tested the scanner; E4 builds the airline desk.
...
```mermaid
flowchart LR
    subgraph P["AppProject storefront — the rulebook"]
        R1["May read from:<br/>storefront-gitops"]
        R2["May deploy to:<br/>workload cluster, namespace storefront-dev"]
        R3["May create:<br/>ConfigMap, Service, Deployment, Job, HPA"]
    end
    A["Application storefront-dev — one delivery order<br/>chart charts/storefront + dev values<br/>to storefront-dev on the workload cluster"]
    A -- "checked against every rule" --> P
    P -- "all rules pass" --> OK["Argo CD renders and compares<br/>(OutOfSync + Missing)"]
    P -- "any rule fails" --> NO["Refused before anything reaches the cluster"]
```
...
- **`namespaceResourceWhitelist`** (project): the kinds the chart renders (Deployment, Service, ConfigMap, a migration Job, an HPA). **Each entry needs two fields, `group` and `kind`**, because Kubernetes files every kind under an API group (a named family of related kinds). The group is the part of `apiVersion` before the `/`; kinds whose `apiVersion` is plain `v1` belong to the core group, written as `""`.
...
| Kind the chart renders | Its `apiVersion` | `group` to write |
|---|---|---|
| ConfigMap | `v1` | `""` |
| Service | `v1` | `""` |
...
The shape of one entry (write one like it for each row):
...
> **💡 Analogy — a guest list with surnames.** The allow-list is a guest list that checks first name *and* surname. `Deployment` in the `apps` family gets in; a guest called `Deployment` with no surname does not match the list, and the doorman turns them away.
...
5. **Check the rulebook without deploying anything.** A mistake in `namespaceResourceWhitelist` does *not* change the `OutOfSync`/`Missing` result, so it would stay hidden until Lab 3. A **dry run** asks Argo CD to walk through a sync and report what it *would* do, while changing nothing on the cluster:
   ```bash
   argocd app sync storefront-dev --dry-run
   ```
   **Expected** when your project is correct *(trimmed)* — every row ends in `(dry run)`:
   ```text
   Message:            successfully synced (no more tasks)

   GROUP  KIND        NAMESPACE       NAME                  STATUS     HEALTH   HOOK     MESSAGE
   batch  Job         storefront-dev  storefront-migration  Succeeded  Synced   PreSync  job.batch/storefront-migration created (dry run)
          ConfigMap   storefront-dev  storefront            OutOfSync  Missing           configmap/storefront created (dry run)
          Service     storefront-dev  storefront            OutOfSync  Missing           service/storefront created (dry run)
   apps   Deployment  storefront-dev  storefront            OutOfSync  Missing           deployment.apps/storefront created (dry run)
   ```
   **If a kind is missing or has the wrong group**, the message names it instead, for example `resource apps:Deployment is not permitted in project storefront`. Fix that entry using the table above, commit, re-apply the project, and run the dry run again. The app stays `OutOfSync`/`Missing` either way: a dry run deploys nothing.
...
- *Hint 4:* The `namespaceResourceWhitelist` must include every kind the chart renders, each with its correct `group`, or Lab 3's sync will be blocked by the fence. Step 5's dry run catches this now.

**Success criterion:** `argocd app get storefront-dev` shows `Sync Status: OutOfSync from main (…)` (your commit ID differs) and `Health Status: Missing`, the diff shows all resources as newly added, and the dry run reports `successfully synced (no more tasks)`. Your one-sentence explanation says *why*: Git describes resources not deployed yet, and sync is manual on purpose.
