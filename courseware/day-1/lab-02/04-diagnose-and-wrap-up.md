...
Each file still has a placeholder, like the templates you used: record 1 contains `<PASSWORD>`, record 2 contains `<TOKEN>`. Fill the placeholder the same way you did for its working twin, so the one deliberate mistake is the only wrong field.
...
2. **Render and apply it.** Reuse your E1 approach for record 1 (fill `<PASSWORD>` from `~/course/credentials/gitea-student.txt`) and your E2 approach for record 2 (fill `<TOKEN>` only; this file has no `<CA_DATA>`). Pipe the result into:
   ```bash
   kubectl --context k3d-mgmt apply --server-side -f -
   ```
   > **Why not apply the file as it is?** Record 1 would then send the literal text `<PASSWORD>` as the password. Argo CD would report a *password* problem instead of the problem this record is about, and the file would have two wrong fields.
3. **Observe.**
...
If all five hold, you have built the real topology, and `reset-lab.sh CP-lab-03 --verify-only --local` prints `PASS CP-lab-03 is in the expected state.` That checkpoint is the starting state for the next lab. It checks the objects, not every line in them. Two small differences are expected: your `storefront` project allows only `storefront-dev`, while the checkpoint's copy also allows `storefront-staging` and `storefront-prod`; and Gitea holds your own commit rather than the checkpoint's.
...
Refresh `storefront-dev`. The sync status is **`Unknown`** — not `OutOfSync`, not `Degraded`. Argo CD is reporting it **can no longer observe live state at all**; absence of evidence renders as `Unknown`, the single most misdiagnosed status. Two more things to notice. The health badge can read **Healthy** even though nothing is deployed: with no live view, that badge is not a real measurement. And Settings → Clusters keeps saying **Successful** for about a minute before it changes to **Failed** with *"the server has asked for the client to provide credentials"*.

Restore it one of two ways. The quick way is to re-run your E2 Move B and Move C commands, which rewrite `cluster-workload` with the real token and CA. The slower way is `reset-lab.sh CP-lab-03 --local`, which also replaces your `platform-config` commits with the checkpoint's copy. Refresh again: status returns to `OutOfSync` / `Missing`. **`Unknown` was never a statement about the app — it was a statement about Argo CD's eyesight.**
