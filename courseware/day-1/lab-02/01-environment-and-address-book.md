...
Your terminal can reach the workload cluster. **That does not mean Argo CD can.** Your terminal signs in with your kubeconfig (the file `kubectl` reads for cluster addresses and credentials). Argo CD is a separate program with its own list of clusters and its own credentials, and nothing copies your access into that list.

> **💡 Analogy — your phone contacts versus a friend's.** Having a number saved in *your* phone does not put it in your friend's phone. Argo CD keeps its own contacts list. The next commands open that list.
...
| What you checked | What it tells you |
| --- | --- |
| Nodes returned from `k3d-workload` | The workload cluster exists, and **your terminal** can reach it |
| Only `in-cluster` in Argo CD's cluster list | **Argo CD** knows its own management cluster, but has no workload-cluster entry yet |

In Module 2, E2, you create a workload identity with limited permissions and store its connection details in Argo CD’s `cluster-workload` Secret.
