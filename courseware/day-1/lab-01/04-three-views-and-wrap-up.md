...
> - **In plain words:** one object exists in three forms. Knowing which form you are looking at stops you from calling a normal difference "drift".
> - **Think of it like:** a recipe (desired: `values.yaml` plus the chart's templates), tonight's printed order ticket with every blank filled in (rendered), and the plate on the table (live). The restaurant staples a table number and a time to the plate. Those stamps were never in the recipe, and nobody would call the dish "wrong" because of them.

```mermaid
flowchart LR
    D["DESIRED<br/>Git: chart templates + values.yaml"] -- "Helm renders" --> R["RENDERED<br/>complete YAML Argo CD would apply"]
    R -- "Sync applies" --> L["LIVE<br/>the object in the cluster,<br/>plus fields Kubernetes adds"]
    L -. "Argo CD compares live with rendered<br/>(ignoring Kubernetes' own stamps)" .-> R
```
...
**Hints:**
- *Hint 1:* Kubernetes stamps every object with bookkeeping at creation. Look under `metadata` on the live object for fields no human could have written in Git (`uid`, `resourceVersion`, `creationTimestamp`). You will also see a `kubectl.kubernetes.io/last-applied-configuration` annotation, a copy of what was last applied. (Kubernetes keeps one more bookkeeping field, `managedFields`, but `kubectl get -o yaml` hides it unless you add `--show-managed-fields`.)
...
> **💡 Analogy — checking on the kitchen three ways.** The web interface is the head chef's clipboard: everything at a glance, nicely laid out. The `argocd` CLI is the intercom: you ask a precise question and get a precise answer. `kubectl` is walking into the kitchen yourself: you see the raw truth, including things the head chef does not track (Pod events), but nobody summarises it for you. When the clipboard is lost (the UI is down), the other two still work.
...
|---|---|---|
| **Nothing happens for about a minute** (sometimes a little longer) after `git push` | The reconciliation timer (`~60s`) has not fired. **Not** a failure. | Wait, or click **Refresh**. |
...
| `curl: (7) Failed to connect to localhost port 9898` | The port-forward stopped (it is attached to one Pod; a rollout ends it). | Run the port-forward lines again. |
| `curl` prints nothing, and `cat /tmp/pf.log` shows `Unable to listen on port 9898` | An older port-forward is still running and holding port 9898, often still connected to a Pod that no longer exists. | `pkill -f "port-forward svc/hello-reconcile"`, then run the port-forward lines again. |
...
<details>
<summary>Show what happens (after you have tested it)</summary>

The app reports **`Synced`** and **`Healthy`**, and `kubectl --context k3d-mgmt -n hello get deploy` shows `0/0`. Health asks "is it doing what was asked?", and zero running copies is exactly what was asked. In kitchen terms: a kitchen told to close for the night is not broken because nobody is cooking. This is also why `Healthy` alone does not prove users are being served — it proves the cluster matches the request.
</details>
...
You watched one change move from Git to a running workload, on the **management cluster** as a deliberate teaching shortcut. Production platforms avoid that. Next, **[Session 3 — Production-Oriented Configuration](../session-03/README.md)** shapes a real platform, and **Lab 2** has you register a *separate* workload cluster and connect a *private* repository (contrast with today's public-read one).

**Before you move on:** no cleanup is required. Lab 2 begins by *checking* its starting state with `reset-lab.sh CP-lab-02 --verify-only --local`, which changes nothing, and your Lab 1 work normally passes that check. Only if a row says `FAIL` does Lab 2 ask you to run a full reset (`reset-lab.sh CP-lab-02 --local`). That reset **discards your Lab 1 commits**: it moves `hello-reconcile`'s `main` branch in Gitea back to the starting chart, resets your `~/hello-reconcile` clone to match, and syncs the app back to "revision one". If you want to keep a record of your checksum fix, copy `chart/templates/deployment.yaml` somewhere outside the clone first.
